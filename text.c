...
void renderResults() {
    ...
    if (playerNum > 0) {
        ...
        drawStringToScreen(
            ...
            SCREEN_HEIGHT / 2 - (CHAR_HEIGHT * WIN_TEXT_SCALE) / 2,
            winMessage, WIN_TEXT_SCALE, 0, COLOR_YELLOW
        );
    }
...
void renderPause() {
    ...
    drawStringToScreen(
        ...
        question, INFO_TEXT_SCALE, 0, COLOR_YELLOW
    );

    drawStringToScreen(
        0, SCREEN_HEIGHT - CHAR_HEIGHT * NAV_TEXT_SCALE,
        "YES", NAV_TEXT_SCALE, 0, COLOR_BLUE
    );
    drawStringToScreen(
        SCREEN_WIDTH - stringWidth("NO") * NAV_TEXT_SCALE,
        SCREEN_HEIGHT - CHAR_HEIGHT * NAV_TEXT_SCALE,
        "NO", NAV_TEXT_SCALE, 0, COLOR_RED);
}
...
void renderScore() {
    ...
    drawStringToScreen(
        SCREEN_WIDTH / 2 - (OFFSET1 + SCORE_TEXT_SCALE * stringWidth(p)),
        OFFSET2, p, SCORE_TEXT_SCALE, 0, COLOR_RED
    );
...
void renderCountdown() {
    drawChar(
        ...
        (char)currentCountdownState + '0', COLOR_GREEN, currentCountdownScale
    );
}
