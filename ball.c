...
char handleCollision(ball_t *ball, pads_t *pads)
{
    ...
    for (int i = 0; i < ball->speed; i++)
    {
        ...
        if (
            yCheck >= pads->p1Pos && yCheck < pads->p1Pos + PAD_HEIGHT && xCheck < PAD_X_OFFSET + PAD_WIDTH)
        {
            ledPulse(0b111, 1);
            ...
            yCheck >= pads->p2Pos && yCheck < pads->p2Pos + PAD_HEIGHT && xCheck >= SCREEN_WIDTH - PAD_X_OFFSET - PAD_WIDTH)
        {
            ledPulse(0b111, 2);
            retValue = V_COLLISION;
