...
void pongText() {
    ...
    if (menu.titleState > 0) {
        drawChar(
            ...
            0,
            'P',
            pongColors[(menu.pongTextShift) % 4],
            PONG_TEXT_SCALE
        );
    }
    ...
    if (menu.titleState > 1) {
        drawChar(
            ...
            0,
            'O',
            pongColors[(menu.pongTextShift + 1) % 4],
            PONG_TEXT_SCALE
        );
    }
    ...
    if (menu.titleState > 2) {
        drawChar(
            ...
            0,
            'N',
            pongColors[(menu.pongTextShift + 2) % 4],
            PONG_TEXT_SCALE
        );
    }
    ...
    if (menu.titleState > 3) {
        drawChar(
            ...
            0,
            'G',
            pongColors[(menu.pongTextShift + 3) % 4],
            PONG_TEXT_SCALE
        );
    }

    if (
        (menu.ticker % PONG_TEXT_ANIMATION_SPEED == 0) &&
...
void renderButtons() {
    drawStringToScreen(
        ...
        MENU_TEXT_SCALE_NORMAL, 0, COLOR_WHITE
    );
    ...
    drawStringToScreen(
        ...
        MENU_TEXT_SCALE_SELECTED, 0, COLOR_YELLOW
    );
    ...
    drawChar(
        ...
        0x5e, COLOR_GREEN, navigationScaleCenter
    );

    drawChar(
        0, MENU_TEXT_Y_OFFSET - MENU_TEXT_SCALE_SELECTED * (CHAR_HEIGHT / 2),
        0x3c, COLOR_RED, navigationScaleLeft
    );

    drawChar(
        SCREEN_WIDTH - charWidth(0x5e) * navigationScaleRight,
        MENU_TEXT_Y_OFFSET - MENU_TEXT_SCALE_SELECTED * (CHAR_HEIGHT / 2),
        0x3e, COLOR_BLUE, navigationScaleRight
    );
}
